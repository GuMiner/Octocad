...
inline bool ShapeManipulator::IsBitFilled(int xp, int yp, CsBitPlane &bitPlane)
{
...
void ShapeManipulator::ExecuteOperation()
{
    ...
    if (currentOperation.hasExtrudeSettings)
    {
        ...
        gm::vec3 startPosition (-5, -5, -5);
        gm::vec3 xBoard (0.05f, 0, 0), yBoard (0, 0.05f, 0), zBoard(0, 0, 0.05f); // Movement to go when moving x, y, and z in terms of the bit plane.
        ...
        for (int i = 0; i < currentOperation.bitPlane.width*2; i++)
        {
            for (int j = 0; j < currentOperation.bitPlane.height*2; j++)
            {
                ...
                for (int k = 0; k < currentOperation.extrudeSettings.distance/resolution; k++)
                {
                    if (isFilled)
                    {
                        gm::vec3 currentPos = startPosition + xBoard*(float)i + yBoard*(float)j + zBoard*(float)k;
